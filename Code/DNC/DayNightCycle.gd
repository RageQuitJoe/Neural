...
export (float) var day_duration = 24 # In minutes
export (float) var day_start_hour = 0 # 24 hours time (0-23)
export (float) var day_start_number = 0

export var color_dawn = Color("ffffa6")
export var color_day = Color("e6e6e6")
export var color_dusk = Color("ff7170")
export var color_night = Color("050505")

export (float) var state_dawn_start_hour = 4
export (float) var state_day_start_hour = 8
export (float) var state_dusk_start_hour = 16
export (float) var state_night_start_hour = 20
...
func load_cycle(new_cycle): #/Game/Save.load_game()
	if new_cycle == 0:
		$Tween.interpolate_property(self, "color", color_dusk, color_night, 0, Tween.TRANS_SINE, Tween.EASE_OUT)
		$Tween.start()

	if new_cycle == 1:
		$Tween.interpolate_property(self, "color", color_night, color_dawn, 0, Tween.TRANS_SINE, Tween.EASE_OUT)
		$Tween.start()

	if new_cycle == 2:
		$Tween.interpolate_property(self, "color", color_dawn, color_day, 0, Tween.TRANS_SINE, Tween.EASE_OUT)
		$Tween.start()

	if new_cycle == 3:
		$Tween.interpolate_property(self, "color", color_day, color_dusk, 0, Tween.TRANS_SINE, Tween.EASE_OUT)
		$Tween.start()
...
func save():
	var save_dict = {
		...
		"current_day_hour" : current_day_hour,
